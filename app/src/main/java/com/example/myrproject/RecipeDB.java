...
public class RecipeDB extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        //데이터 베이스가 생성될 때 호출
        //데이터 베이스 -> 테이블 -> 컬럼 -> 값
        db.execSQL("CREATE TABLE IF NOT EXISTS Refrigerator(id INTEGER PRIMARY KEY AUTOINCREMENT, food TEXT NOT NULL, info TEXT NOT NULL, type TEXT NOT NULL, name TEXT NOT NULL, cnt TEXT NOT NULL, unit Text NOT NULL, writedate TEXT NOT NULL)");//만약 존재하지 않으면 테이블 만들어라(쿼리)
        ...
        //음식이름 food, 설명 info, 재료종류 type, 재료 이름 name, 수량 cnt, 단위 unit
    }
