...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        //데이터 베이스가 생성될 때 호출
        //데이터 베이스 -> 테이블 -> 컬럼 -> 값
        db.execSQL("CREATE TABLE IF NOT EXISTS refrigerator(id INTEGER PRIMARY KEY AUTOINCREMENT, type TEXT NOT NULL, name TEXT NOT NULL, cnt TEXT NOT NULL, unit TEXT NOT NULL)");
    ...
    public ArrayList<refrigeitem> getrefrigerator(){//메소드로 만든다
        ...
        if(cursor.getCount()!=0){//반드시 데이터가 있다는 사실
            ...
            while(cursor.moveToNext()){
                ...
                refrigeItem.setCnt(cnt);
                refrigeItem.setUnit(unit);
    ...
    //냉장고 물품 추가
    public void InsertItem(String _type, String _name, int _cnt, String _unit) {
        //id는 안넣나? -> 보통 아이디는 생략가능 자동으로 가능 //public은 어디서나 삽입 가능하게
        SQLiteDatabase db = getWritableDatabase();//쓰기 가능한
        db.execSQL("INSERT INTO refrigerator(type,name,cnt,unit) VALUES('"+_type+"','"+_name+"','"+_cnt+"','"+_unit+"');");
